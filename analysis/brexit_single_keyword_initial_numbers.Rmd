...
```{r, message=F}
brexit_data <- dir_ls(here::here("data/initial_batch_job")) %>% 
  map_df(read_csv)
...
7 rows have been dropped. Except the last days of each month, all remaining rows should have exactly the same values as the earlier dataset. I'll test this by anti-joining the processed dataset and raw dataset while using the day_ratio column, which I have just recalculated for the former, as key.
...
initial_plot_hs_faceted <- brexit_data_processed %>% 
  ...
  scale_x_continuous(labels = seq(1,31,1), breaks = seq(1,31,1))+
  labs(title="Tweets classified as anti-Muslim hate speech sent per day", 
       subtitle= "Data: Single keyword Brexit Dataset")+
...
Since all the above looks good
```{r}
brexit_data_processed %>%
